...
function arg_str(arg, dicts) {
    var argstr = arg;

    while (true) {
        var m = bracket_pre_compiled.exec(arg);
        if (!m)
            break;
        ...
        if (arg_value)
            argstr = argstr.replace(m[0], arg_value);
    }
...
class launch {
    constructor(option) {
        this.defaultJavaParameter = [
            ...
            "-XX:G1HeapRegionSize=16M"].join(" ");
        if (mrule.osname == "osx")
            this.defaultJavaParameter += " -XstartOnFirstThread";
    ...
    createArg() {
        ...
        var jvmdict = {
            "natives_directory": e(profile.minecraft.natives),
        ...
        if (profile.jvm_arguments && profile.jvm_arguments.length > 0)
            ...
            args.push("-Djava.library.path=" + e(profile.minecraft.natives));
            args.push("-cp " + libs);
        }
        ...
        var gamedict = {
            ...
            "game_directory": e(profile.minecraft.path),
            "assets_root": e(profile.minecraft.assets),
            ...
            "user_properties": "{}",
            "user_type": "Mojang",
            "game_assets": e(profile.minecraft.assetLegacy),
        ...
        if (profile.game_arguments && profile.game_arguments.length > 0) // 1.3
            ...
            args.push(arg_str(profile.minecraftArguments, gamedict));

        // options
        if (this.launchOption.server_ip)
            args.push("--server " + this.launchOption.server_ip)

        if (this.launchOption.screen_width && this.launchOption.screen_height) {
            args.push("--width " + this.launchOption.screen_width)
            args.push("--height " + this.launchOption.screen_height)
        }
